...
利用シーンによっては追加の外部ライブラリが必要になります (= 例: HTTP/3 機能の QUIC 実装、PHP 版での PHP ランタイム連携)。それでも `tar` / `jar` を展開すれば動く、という軽さは維持しています。
...
いずれも **1 スレッドが切れ目なく動き続ける** 設計で、CPU コア数程度のスレッド (= Go なら goroutine プール) だけで機材の上限まで性能を引き出せます。
...
- **Version 4** — 開発中 ([What's New](whats-new.md) 参照)
